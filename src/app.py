...
class SignalGeneratorApp(tk.Tk):
    ...
    def _load_templates(self) -> dict:
        if not CONFIG_DIR.exists():
            raise FileNotFoundError(f"Не найдена папка конфигурации: {CONFIG_DIR}")

        config_files = sorted(CONFIG_DIR.glob("*.json"))
        if not config_files:
            raise FileNotFoundError(f"Нет JSON-файлов в папке конфигурации: {CONFIG_DIR}")

        merged = {"defaults": {}, "types": {}}
        for config_path in config_files:
            ...
            for section, values in defaults.items():
                merged["defaults"].setdefault(section, {}).update(values)

            merged["types"].update(config.get("types", {}))
