...
def di_signals_for_voltage(connection: Connection) -> List[Tuple[str, str]]:
    base = [
        ("Включен", "ТС"),
        ("Отключен", "ТС"),
        ("Управление местное", "ТС"),
        ("Неисправность привода", "АПТС"),
    ]
    if connection.voltage_class < 35:
        base.extend(
            [
                ("Неисправность РЗА", "АПТС"),
                ("Аварийное отключение", "АПТС"),
            ]
        )
        if connection.connection_type == "ТН":
            base.extend(
                [
                    ("АВ цепей ТН отключен", "АПТС"),
                    ("Земля в сети", "АПТС"),
                ]
            )
    return base
...
def di_do_place(connection: Connection, apparatus_name: str) -> str:
    ...
    return "РУ-6(10) кВ. Привод {apparatus}".format(apparatus=apparatus_name)
...
def di_rows(connection: Connection, apparatus_counts: Dict[str, int]) -> List[Dict[str, str]]:
    ...
    header_name = f"{connection.name} ({connection.connection_type}; {format_apparatus_summary(apparatus_counts)})"
    for apparatus_name in apparatus_list:
        for signal_name, signal_type in di_signals_for_voltage(connection):
            rows.append(
                {
                    "Наименование присоединения": header_name,
                    ...
                    "Место съема сигнала": di_do_place(connection, apparatus_name),
                    "Устройство регистрации сигнала": "",
                }
            )
    return rows
...
def do_rows(connection: Connection, apparatus_counts: Dict[str, int]) -> List[Dict[str, str]]:
    ...
    header_name = f"{connection.name} ({connection.connection_type}; {format_apparatus_summary(apparatus_counts)})"
    for apparatus_name in apparatus_list:
        for signal_name in ["Включить", "Отключить"]:
            rows.append(
                {
                    "Наименование присоединения": header_name,
                    "Наименование аппарата": apparatus_name,
